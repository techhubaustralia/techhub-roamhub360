...
import { reminderEmail, checkInEmail, checkOutEmail } from "@/lib/server/email";
...
type Task = "reminder" | "checkin" | "checkout" | "auto-release" | "auto-checkout";
...
async function liveBuildings(): Promise<{ id: string; iana: string }[]> {
  const hidden = new Set(await listHiddenBuildings());
  const buildings = (await listCustomBuildings()).filter((b) => !hidden.has(b.id));
  const out: { id: string; iana: string }[] = [];
  for (const b of buildings) {
    const plan = await getStoredPlan(b.id);
    out.push({ id: b.id, iana: plan?.tz || "UTC" });
  }
  return out;
}

async function runTask(task: Task, buildingRoot: string, localDate: string, localNow: string, all: Booking[]): Promise<number> {
...
export async function GET(req: Request, { params }: { params: Promise<{ task: string }> }) {
  ...
  if (task === "tick") {
    for (const o of buildings) {
      ...
      for (const t of Object.keys(TARGET) as Task[]) {
        if (TARGET[t] === hhmm) results[`${o.id}:${t}`] = await runTask(t, o.id, date, localNow, all);
      }
    }
  ...
  for (const o of buildings) {
    const { date, hhmm } = officeNow(o.iana);
    results[o.id] = await runTask(task as Task, o.id, date, `${date}T${hhmm}`, all);
  }
